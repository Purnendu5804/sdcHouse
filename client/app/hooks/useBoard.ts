...
export const useBoard = ({BOARD_SIZE ,STEP_SIZE , DOT_SIZE , onMove , hasJoined} : UseBoardProps) => {
    ...
    useEffect(() => {
        ...
        setPosition((prev) => {
            ...
            if (e.key === "ArrowDown") { newY = Math.min(BOARD_SIZE - DOT_SIZE, prev.y + STEP_SIZE); newDir = 'down'; }
            if (e.key === "ArrowLeft") { newX = Math.max(0, prev.x - STEP_SIZE); newDir = 'left'; }
            if (e.key === "ArrowRight") { newX = Math.min(BOARD_SIZE - DOT_SIZE, prev.x + STEP_SIZE); newDir = 'right'; }
